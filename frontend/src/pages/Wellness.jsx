...
export const allCards = [
  {
    title: "Prenatal Stretching for 1st Trimester",
    tags: ["Exercise", "1st Trimester"],
    time: "10 mins",
    category: "Exercise",
  ...
  {
    title: "DIY Relaxing Bath Ritual",
  ...
  {
    title: "Spinach & Quinoa Power Bowl",
    tags: ["Recipe", "Iron-Rich"],
    time: "25 mins",
    category: "Recipe",
    trimester: "All",
    fitness: "All",
  ...
  {
    title: "Water Workouts for Late Pregnancy",
    tags: ["Exercise", "3rd Trimester"],
    time: "45 mins",
    category: "Exercise",
...
const Wellness = () => {
  ...
  useEffect(() => {
    const preloadImages = [yogaImage, meditateImage, foodsImage, careImage];
  ...
  const applyFilters = () => {
    ...
    if (filters.trimester !== "All") {
      cards = cards.filter(
        (c) => c.trimester === filters.trimester || c.trimester === "All",
      );
    }
    ...
    if (filters.fitness !== "All") {
      cards = cards.filter(
        (c) => c.fitness === filters.fitness || c.fitness === "All",
      );
    }
        ...
        <main className="min-h-screen pt-32 px-8 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto">
            <section className="bg-white/40 backdrop-blur-md rounded-lg shadow p-6 text-center">
              <h1 className="text-3xl font-bold text-[#234451]">
                A healthy baby starts with you.
              </h1>
              <p className="mt-2 text-lg text-[#234451]">
                From pregnancy safe recipes to gentle exercises, we've got you
                covered.
              </p>
              <div className="mt-4 flex justify-center gap-2 flex-wrap">
                <span className="bg-[#DFA69F] text-[#234451] text-xs px-2 py-1 rounded-full">
            ...
            <div className="flex flex-col lg:flex-row mt-8 gap-8">
              <aside className="lg:w-1/4 space-y-6">
                <div className="bg-white/40 backdrop-blur-md rounded-lg shadow p-4">
                  ...
                  <ul className="mt-2 space-y-1 text-[#234451] text-sm">
                    {["Recipe", "Exercise", "Meditation", "Self-Care"].map(
                      (cat, i) => (
                        <li
                          key={i}
                          onClick={() => {
                ...
                <div className="bg-white/40 backdrop-blur-md rounded-lg shadow p-4">
                  ...
                  >
                    ...
                    <span className="text-[#234451] text-xl">
                      {showFavorites ? "–" : "+"}
                    </span>
                  </div>
                  {showFavorites && (
              ...
              <div className="flex-1 space-y-6">
                <div className="flex gap-2 flex-wrap">
                  <select
                    onChange={(e) =>
                      setFilters({ ...filters, trimester: e.target.value })
                    }
                    className="border border-[#EAE3CA] rounded px-2 py-1 text-sm text-[#234451] bg-white/20 backdrop-blur-md"
                  >
                    <option>All</option>
                  ...
                  <select
                    onChange={(e) =>
                      setFilters({ ...filters, fitness: e.target.value })
                    }
                    className="border border-[#EAE3CA] rounded px-2 py-1 text-sm text-[#234451] bg-white/20 backdrop-blur-md"
                  >
                    <option>All</option>
                  ...
                  <button
                    ...
                    className="bg-[#DFA69F] text-[#234451] px-4 py-1 rounded text-sm font-bold hover:bg-[#e8bbb5] transition"
                  >
                    Apply
                  </button>
                </div>
                      ...
                      <div className="p-4 space-y-2">
                        <div className="flex gap-2 flex-wrap text-xs text-[#fff]">
                          {card.tags.map((t, j) => (
                            <span
                              key={j}
                              className="bg-[#99C8C1] text-[#234451] px-2 py-0.5 rounded"
                            >
                              {t}
                            </span>
                          ))}
                        </div>
                        <h3 className="font-semibold text-[#234451] min-h-[3rem]">
                          {card.title}
                        </h3>
                        <div className="flex justify-between items-center">
                          <div className="text-xs text-[#234451]">
                            {card.time}
                          </div>
                          <button
                            onClick={(e) => {
                ...
                {visibleCount < filteredCards.length && (
                  <div className="flex justify-center">
                    <button
                      ...
                      className="bg-[#DFA69F] text-[#234451] px-6 py-2 rounded text-sm font-bold hover:bg-[#e8bbb5] transition"
                    >
                      Load More
                    </button>
                  </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg text-center shadow-lg space-y-4 relative w-80">
            ...
            <p className="text-sm text-[#234451]">
              You need to be logged in to access wellness content.
            </p>
            <a
              href="/login"
      ...
      <div className="max-w-3xl mx-auto bg-white/40 backdrop-blur-md shadow rounded-lg overflow-hidden">
        ...
        <div className="p-6">
          ...
          <div className="mt-2 text-sm text-[#234451] space-x-2">
            <span className="bg-[#99C8C1] px-2 py-0.5 rounded">
              {card.category}
            </span>
            <span className="bg-[#99C8C1] px-2 py-0.5 rounded">
              {card.trimester}
          ...
          <p className="mt-4 text-[#234451]">
            Detailed content for this post will go here. You can add
            instructions, ingredients, routines, etc.
          </p>
        </div>
      </div>
    </div>
